...
import eda.base as base

def describe_numeric_1d(series, **kwargs):
    # Format a number as a percentage. For example 0.25 will be turned to 25%.
    _percentile_format = "{:.0%}"
    stats = dict()
    stats['correlation_var'] = None
...
#    stats['variance'] = series.var()
    stats['coef_variation'] = stats['std'] / stats['mean'] if stats['mean'] else np.NaN
...
def describe_categorical_1d(series, **kwargs):
    ...
    if base.get_vartype(series) == base.TYPE_CAT:
        names += ['top', 'freq', 'type', 'correlation_var']
        result += [top, freq, base.TYPE_CAT, 'NotTested']
...
def describe_supported(series, **kwargs):
    ...
    results_data = {'n_noNA_noInf': count,
...
def describe(df, check_correlation=True, correlation_threshold=0.9, correlation_overrides=None, check_recoded=False, pool_size=multiprocessing.cpu_count(), **kwargs):
    ...
    print("univariate summary finished")
	
	
	
	# Convert ldesc to a DataFrame
    names = []
	# name of summary output table columns
    ldesc_indexes = sorted([x.index for x in ldesc.values()], key=len)
	# loop through variables
    for idxnames in ldesc_indexes:
		#loop through summary stats for a variable
        for name in idxnames:
            if name not in names:
				# get all possible summary stats names
                names.append(name)
    variable_stats = pd.concat(ldesc, join_axes=pd.Index([names]), axis=1)
    variable_stats.columns.names = df.columns.names
    var_num = variable_stats.columns[variable_stats.loc['type',:] == 'NUM']
	
	
	# Get correlations
    dfcorrPear = np.corrcoef(df.loc[:,var_num].values,rowvar = False)
    dfcorrPear = pd.DataFrame(dfcorrPear,columns = var_num,index = var_num)
#    dfcorrSpear = df.corr(method="spearman")
    print("correlation matrix finished")
        ...
        for x, corr_x in corr.iterrows():
            ...
            for y, corr in corr_x.iteritems():
                ...
                if corr > correlation_threshold:
                    variable_stats.loc['type',x] = 'CORR'
                    variable_stats.loc['correlation_var',x] = y
                    variable_stats.loc['correlation',x] = corr
        if check_recoded:
            ...
            for (name1, data1), (name2, data2) in itertools.combinations(categorical_variables, 2):
                ...
                if confusion_matrix.values.diagonal().sum() == len(df):
                    variable_stats.loc['type',x] = 'RECODED'
                    variable_stats.loc['correlation_var',x] = name2
    print("correlated var report finished")
